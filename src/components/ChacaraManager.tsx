...
export const ChacaraManager: React.FC<ChacaraManagerProps> = ({ fetchWithAuth, activeTab, onDataUpdate, setActiveTab }) => {
  ...
  useEffect(() => {
    ...
    if (preFill && users.length > 0) {
        ...
        const overrides = {
            ...
            energy_readings: log.energyReadings.map((r: string) => ({ curr: Number(r) })),
            water_readings: log.waterReadings.map((r: string) => ({ curr: Number(r) }))
        };
        handleUserSelectForBill(String(log.userId), overrides);
  ...
  const handleUserSelectForBill = (userId: string, overrides?: any) => {
    ...
    if (user) {
      ...
      const lastBillEnergy = user.last_bill?.energy_readings || [];
      
      for (let i = 0; i < energyCount; i++) {
        let prev = 0;
        if (lastBillEnergy[i]) {
            prev = lastBillEnergy[i].curr;
        } else if (user.last_bill) {
            // Fallback to legacy fields for first two meters
            if (i === 0) prev = user.last_bill.curr_reading || 0;
            else if (i === 1) prev = user.last_bill.curr_reading_2 || 0;
        ...
        const overrideCurr = overrides?.energy_readings?.[i]?.curr;
        const curr = overrideCurr !== undefined ? overrideCurr : 0;
      ...
      const lastBillWater = user.last_bill?.water_readings || [];
      
      for (let i = 0; i < waterCount; i++) {
        let prev = 0;
        if (lastBillWater[i]) {
            prev = lastBillWater[i].curr;
        } else if (user.last_bill) {
            // Fallback to legacy fields for first two meters
            if (i === 0) prev = user.last_bill.water_curr_reading || 0;
            else if (i === 1) prev = user.last_bill.water_curr_reading_2 || 0;
        ...
        const overrideCurr = overrides?.water_readings?.[i]?.curr;
        const curr = overrideCurr !== undefined ? overrideCurr : 0;
      ...
      setBillForm(prev => ({
        ...
        include_apportionment: true,
        reading_date: overrides?.reading_date || readingDate.toISOString().split('T')[0],
